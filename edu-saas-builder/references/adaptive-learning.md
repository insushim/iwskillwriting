...
## 실전 교훈: 적응형 콘텐츠 볼륨 (v3.2)

> 매쓰버스 프로젝트 실전 피드백 기반 (2026.04)

### 문제: 콘텐츠 부족
- 단원당 2-3개 스킬 → 사용자 "이게 다야?" 반응
- 연습 없이 개념만 → 정답률이 안정되지 않음

### 해결: 개념-연습-심화 패턴 (단원당 7-8스킬)
```yaml
1. 개념 도입 (0.3)   — "계산 순서"
2. 개념 연습 (0.4)   — "계산 순서 연습"     ← 반복 훈련
3. 심화 개념 (0.5)   — "괄호가 있는 식"
4. 심화 연습 (0.6)   — "괄호 식 연습"       ← 반복 훈련
5. 실전 적용 (0.7)   — "혼합 계산 실전"
6. 도전 (0.8)        — "혼합 계산 도전"
7. 문장제 (0.8)      — "혼합 계산 문장제"
```

### 절차적 문제 생성 (ensureMinimumPool)
스킬에 직접 등록된 문제가 5개 미만이면 자동으로 절차적 생성:
```typescript
function ensureMinimumPool(pool, skillId, skillCode, grade, theta, min) {
  if (pool.length >= min) return pool;
  // skillCode에 기반하여 숫자만 바꾼 유사 문제 생성
  // 예: mixed_calc_order → 덧셈/뺄셈/곱셈/나눗셈 조합을 랜덤 생성
  return [...pool, ...generateProceduralQuestions(skillCode, grade, min - pool.length)];
}
```

### 오답 재풀기로 실제 학습 보장
- 10문제 중 오답 → 답 안보여줌 → 세션 끝에 다시 출제
- 정답률이 안정될 때까지 같은 유형 반복
- **핵심**: "답을 외우는 것"이 아니라 "풀이 과정을 익히는 것"
